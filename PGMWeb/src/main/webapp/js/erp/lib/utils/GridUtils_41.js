...
//						break;
	...
	allRecordsToJson : function(grid/*Ext.gird*/, withoutFilter) {
		...
		var records = grid.getStore().data.getRange();
		return CUtils.jsonEncode(this.dataToArray(records));
	},
	...
	setPopupValue : function(record, popupId, popupIdValue, popupName, popupNameValue, prefixId) {
		...
		if(column){
			if(column.field && column.field.getXType() == 'triggerfield'){
	...
	addIntoColumns : function(gColumns,gFields,col, gId){
		col.gridId = gId;
...
//				}
	...
	//end------------------------------------------------------------
	initErpGrid : function(_gId, _gExtraParams, _gConfig) {
		var me =this;
		...
		for (var i = 0, l = gConfig.columns.length; i < l; i ++) {
			...
			if(col.fieldType == 'columns'){
				this.addDynamicColumns(gColumns,gFields,col.value, gId);
			}else{
				this.addIntoColumns(gColumns,gFields, col, gId);
			}
		}
		...
		if(cfg.editable){
			switch(cfg.fieldType){
				case "text" : 
					...
					break;
				case "date":
					cfg.renderer = GUtils.dateRenderer;
					cfg.editor={
						xtype: 'datefield',
					...
					var browerButton = new Ext.form.field.Trigger({
						...
						onTriggerClick: function() {
							if(cfg.popupSelectOnClick) {
								PAction[cfg.popupSelectOnClick](cfg.dataIndex);
							} else {
				...
				case "colorField":
					var trigger = new Ext.form.field.Picker({
						...
						onTriggerClick : function() {
							...
							colorPick.setPosition(trigger.el.getX(),trigger.el.getY()+trigger.getHeight()+1,false);
							colorPick.show();
						}
					});
				    cfg.field = trigger;
